...
export function useSiteAnalysisOwnership() {
  const { state: zoneState } = useZoneSelection();
  const [state, setState] = useState<OwnershipState>(initialState);
  ...
  const loadOwnership = useCallback(async () => {
    ...
    try {
      const response = await fetch("/analysis/basic-info/ownership", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(message || `소유현황 요청에 실패했습니다. (${response.status})`);
      }
      ...
      setState({
        status: "error",
        data: null,
        error: error instanceof Error ? error.message : "소유현황 요청 중 오류가 발생했습니다."
      });
    } finally {
