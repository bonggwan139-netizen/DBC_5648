...
type RoadSideStatus = "idle" | "loading" | "success" | "error";

type RoadSideState = {
  status: RoadSideStatus;
  data: RoadSideResponse | null;
  error: string | null;
};

const initialState: RoadSideState = {
  status: "idle",
  data: null,
  error: null
};

export function useSiteAnalysisRoadSide() {
  const { state: zoneState } = useZoneSelection();
  const [state, setState] = useState<RoadSideState>(initialState);
  const abortControllerRef = useRef<AbortController | null>(null);

  const canRequest = zoneState.status === "confirmed" && zoneState.confirmedZone !== null;

  const loadRoadSide = useCallback(async () => {
    ...
    try {
      const response = await fetch("/analysis/basic-info/road-side", {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(message || `접도구분 요청에 실패했습니다. (${response.status})`);
      }

      const data = (await response.json()) as RoadSideResponse;
      setState({
        status: "success",
        data,
        error: null
      ...
      setState({
        status: "error",
        data: null,
        error: error instanceof Error ? error.message : "접도구분 요청 중 오류가 발생했습니다."
      });
    } finally {
